...
function OverlayAdmin({ isOpen, onClose, editingDoc, viewMode, editMode }) {
  ...
  useEffect(() => {
    if (isSuperAdmin) {
      if (formData.usertype === 'superadmin') {
        ...
        formData.documentdirection === 'all'
      ) {
  ...
  const validate = () => {
    ...
    if (
      !/^([a-zA-Z0-9._-]+)@region1\.dost\.gov\.ph$/i.test(
    ...
    if (
      ...
      newErrors.documentdirection = 'Direction is required.';
    if (isSuperAdmin && !formData.usertype)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const isDuplicate = admins.some(
        (a) =>
          a.adminemail.toLowerCase() === payload.adminemail.toLowerCase() &&
      ...
      if (!response.ok) {
        ...
        setErrors({
          submit:
            err.error?.replace('username', 'email') || 'Failed to save admin.',
        });
        setIsSubmitting(false);
        Swal.fire({
          icon: 'error',
  ...
  const modeLabel = editingDoc
    ? 'View / Edit User'
    : 'Add New User';
  const modeSubtitle = editingDoc
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-slate-900/50 backdrop-blur-sm p-4 modal-backdrop">
      <div
        ref={popupRef}
        className="modal-panel w-full max-w-[750px] flex flex-col"
      ...
      >
        ...
        {/* Form Body */}
        <div className="px-6 py-5 overflow-y-auto max-h-[70vh] scrollbar-hide">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            <div className="modal-field">
              ...
              />
              {errors.adminname && (
                <p className="modal-error-msg">{errors.adminname}</p>
              )}
            </div>
            ...
            <div className="modal-field">
              ...
              />
              {errors.adminemail && (
                <p className="modal-error-msg">{errors.adminemail}</p>
              )}
            </div>
            ...
            {isSuperAdmin && (
              <div className="modal-field">
                ...
                {errors.usertype && (
                  <p className="modal-error-msg">{errors.usertype}</p>
                )}
              </div>
            )}
            ...
            {formData.usertype === 'admin' && editingDoc && (
              <div className="modal-field">
                <label className="modal-label">
                  Document Direction <span className="req">*</span>
                </label>
                <select
                ...
                {errors.documentdirection && (
                  <p className="modal-error-msg">{errors.documentdirection}</p>
                )}
              </div>
            )}
              ...
              <>
                ...
                <div className="modal-field">
                  <label className="modal-label">
                    ...
                    {!editMode && <span className="req">*</span>}
                  </label>
                  <input
                    type={showPassword ? 'text' : 'password'}
                    name="adminpass"
                    placeholder={
                      editMode ? 'Leave blank to keep current' : 'Enter password'
                    }
                  ...
                  />
                  {errors.adminpass && (
                    <p className="modal-error-msg">{errors.adminpass}</p>
                  )}
                </div>
                ...
                <div className="modal-field">
                  ...
                  />
                  {errors.confirmPass && (
                    <p className="modal-error-msg">{errors.confirmPass}</p>
                  )}
                </div>
